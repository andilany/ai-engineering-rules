...
def _render_core() -> str:
    ...
    lines = [
        ...
        "",
        "These are cross-project invariants. Project-specific instructions and explicit user requests take precedence over generic preferences.",
        "",
    ]
    for severity in _ORDER:
        selected = [rules[module] for module in GLOBAL_CORE_MODULES if rules[module].severity is severity]
        if not selected:
            continue
...
def bootstrap(home: Path, *, codex_home: Path | None, dry_run: bool) -> BootstrapResult:
    home = home.resolve()
    ...
    cursor_text = (
        ...
        "Cursor User Rules are managed by Cursor settings; airules does not modify them automatically.\n\n"
        + core
    )
    writes = (
        plan_write(codex, upsert_managed_block(_read(codex), core)),
        plan_write(claude, upsert_managed_block(_read(claude), core)),
        plan_write(gemini, upsert_managed_block(_read(gemini), core)),
        plan_write(cursor, cursor_text),
    )
    scope = WriteScope(
        root=home,
        allowed_exact=frozenset({codex, claude, gemini, cursor}),
    )
    applied = apply_writes(writes, dry_run=dry_run, scope=scope)
    return BootstrapResult(
        writes=applied,
        cursor_note="Cursor: paste ~/.ai-rules/cursor-user-rules.txt into Settings > Rules > User Rules.",
    )
